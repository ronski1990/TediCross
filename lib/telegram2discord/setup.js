	...
	updateEmitter.on("text", wrapFunction(async (message, bridge) => {
		...
		try {
			...
			if (messageObj.reply !== null) {
				...
				await webhook.send({...messageOptions, embeds: [embed]});
			}

			// Discord doesn't handle messages longer than 2000 characters. Split it up into chunks that big
			const chunks = R.splitEvery(2000, messageObj.text);

			// Send them in serial
			let dcMessage = null;
			for (const chunk of chunks) {
				dcMessage = await webhook.send(chunk, messageOptions);
			}
