...
function setup(logger, dcBot, tgBot, messageMap, bridgeMap, settings) {
	...
	dcBot.on("message", async (message) => {
		...
		// Ignore messages from the bot's webhooks
		if (!R.isNil(message.webhookID)) {
			const webhook = await dcBot.fetchWebhook(message.webhookID);
			if (webhook.name === "TediCross") {
				return;
			}
		}
	...
	dcBot.on("messageUpdate", async (oldMessage, newMessage) => {
		...
		try {
			...
			const tgMessageId = await messageMap.getCorresponding(MessageMap.DISCORD_TO_TELEGRAM, newMessage.id);
